...
#define VAG_SAMPLES_PER_BLOCK 28 // Samples per VAG ADPCM block   VAG_SAMPLES_IN_LINE
#define VAG_BLOCK_SIZE      16 // ADPCM block size in bytes  VAG_LINE_SIZE
    ...
    int& loopStartSample, int& loopEndSample)
{
    ...
    for (size_t b = 0; b < blocks; ++b) {
        ...
        if (flag == 0x03) {
            ...
            //loopEndSample = samplePos + VAG_SAMPLES_PER_BLOCK; // in the end loop
            break;
        }
        if (flag == 0x07) {
            // EOF
